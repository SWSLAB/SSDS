function [T] = Main_data_exposure(case_study, frac)
%% ####################################################################################################################
% Code for the paper:
...
%% Exposure level
if frac ~= 1                                                     % If fraction is 100% do nothing    
    n = length(X_train_dma{1});
    len_exposure = round(n * frac);
    if mod(len_exposure, 24)
        len_exposure = len_exposure - mod(len_exposure, 24);     % Make it dividable by 24
    end
    
    if len_exposure < 24
        cd(startFolder)
        error('frac too small!')
    end
    
    for i = 1:length(X_train_dma)
        X_train_dma{i} = X_train_dma{i}(end - len_exposure + 1:end, :);
        X_train_dma_mod{i} = X_train_dma_mod{i}(end - len_exposure + 1:end, :);
    end
end
...
for i=1:NumDMAs
    ...
    DV_WTA_tst1(:, cnt) = Reduce2D_and_Predict(repmat(tmp1{i},size(X_test1_dma{i},1)/24,1), X_test1_dma{i}, a, b, M, Sigma, model, plotb, name);
    DV_WTA_tst2(:, cnt) = Reduce2D_and_Predict(repmat(tmp1{i},size(X_test2_dma{i},1)/24,1), X_test2_dma{i}, a, b, M, Sigma, model, plotb, name);
    cnt = cnt + 1;
end
...
T.("Perc" + string(frac*100)) = [S_test1, S_TTD_test1, S_cm_test1, F1_test1, Prec_test1, Sens_test1, ...
    S_test2, S_TTD_test2, S_cm_test2, F1_test2, Prec_test2, Sens_test2]';
cd(startFolder)
end
