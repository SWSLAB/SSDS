function [T] = Main_smoothing_window_length(case_study)
% ####################################################################################################################
% Code for the paper:
...
for win = 1:24                                                              % Smoothing window size
    ...
    T.("Win"+string(win)) = [S_test1, S_TTD_test1, S_cm_test1, F1_test1, Prec_test1, Sens_test1, ...
        S_test2, S_TTD_test2, S_cm_test2, F1_test2, Prec_test2, Sens_test2]';    
end

cd(startFolder)
writetable(T, upper(case_study) + "town_smoothing_window_SA.csv", 'WriteRowNames',true)
end
